function Debye3D=Debye3D(Xvalue)
%
% This function calculates the first derivative d[Debye3(x)]/dx of the Debye function of
% order 3, where
%
         ...
         if (abs(D(NTERMS)) > T) 
              if (X <= FOUR) 
                  ...
                  if (X > XLIM2) 
                      ...
                      Debye3D = -THREE./(DEBINF.*X.*X.*X.*X);
                      if (X < XLIM1) 
                          EXPMX = exp(-X);
                          if (X > XUPPER) 
                              SUM = ((((X + 3).*X + 9).*X + 18).*X + 18) / (X.*X.*X.*X);
                          else
                              SUM = ZERO;
                              RK = floor(XLIM1./X);
                              NEXP = floor(RK);
                              ...
                              for i = NEXP:-1:1
                                  XKI = ONE / XK;
                                  T = (((18.*XKI + 18).*XKI + 9).*XKI + 3).*XKI + 1;
                                  SUM = SUM.*EXPMX + T;
                                  RK = RK - ONE;
                                  XK = XK - X;
                              end
                          end
                              Debye3D = Debye3D + THREE.*SUM.*EXPMX;
                      end
                  end
              end
              break
