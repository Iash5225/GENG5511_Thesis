function props = computeThermoPropsForResult(T, p)
    ...
    v00 = 22.555; % Reference molar volume (cm3/mol)
    a1 = 2656.5;
    a2 = 7298;
    a3 = 10;
    a(1) = 3;
    % Initialize arrays and constants
    Th(1) = 86.44; % Characteristic temperatures
    g(1) = 2.68; % Gruneisen parameters
    q(1) = 0.0024; % Exponent for volume dependence of Gamma
    for ii  = 2:6
        Th(ii) = 0; % Characteristic temperatures
        g(ii) = 0; % Gruneisen parameters
        q(ii) = 0; % Exponent for volume dependence of Gamma        
    end
    aa = 0.0128;
    bb = 0.388;
    cc = 7.85;
    ...
    for j = 1:100
        ...
        if abs(vstep) > 0.1 * v00
            vstep = sign(vstep) * 0.2 * v00;
        end
        v = v + vstep;
        if abs(perror) <= eps, break; end
        if v <= 0, v = 100; break; end
    end
