...
def extract_datasets_with_meta(data):
    ...
    delta_H_sub = 1000.0 * \
        pd.to_numeric(data["heatsub"]["Change in Enthalpy"],
                      errors="coerce").to_numpy()
    H_fluid_sub = 1000.0 * \
        pd.to_numeric(data["heatsub"]["Enthalpy"], errors="coerce").to_numpy()

    T_H_melt = data["fusion"]["Temperature"].to_numpy()
    p_H_melt = data["fusion"]["Pressure"].to_numpy()
    delta_H_melt = 1000.0 * \
        pd.to_numeric(data["fusion"]["Change in Enthalpy"],
                      errors="coerce").to_numpy()
    H_fluid_melt = 1000.0 * \
        pd.to_numeric(data["fusion"]["Enthalpy"], errors="coerce").to_numpy()
